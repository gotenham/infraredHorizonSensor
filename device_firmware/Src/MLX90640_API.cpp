...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    // write b00110000 to 0x8000; believe this is bit 5 (Start Of Measurement flag), bit 4
    error = MLX90640_I2CWrite(slaveAddr, MLX90640_STATUS_REG, MLX90640_INIT_STATUS_VALUE);
    if(error == -MLX90640_I2C_NACK_ERROR)
    {
        return error;
...
//------------------------- To calculation -------------------------------------
    ...
    for( int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ...
        // AJ note: check if pixel is at current subpage rendered location (differs chess or interlev)
...
void MLX90640_GetImage(uint16_t *frameData, const paramsMLX90640 *params, float *result)
{
    ...
    subPage = frameData[833];
    vdd = MLX90640_GetVdd(frameData, params); // AJ Note: 11.2.2.2. Supply voltage value calculation (common for all pixels)
    ta = MLX90640_GetTa(frameData, params); // AJ Note: 11.2.2.3. Ambient temperature calculation (common for all pixels)

    ktaScale = POW2(params->ktaScale);
    kvScale = POW2(params->kvScale);

//------------------------- Gain calculation -----------------------------------
    //Confirmed: 11.2.2.4. Gain parameter calculation (common for all pixels)
    gain = (float)params->gainEE / (int16_t)frameData[778];
...
// Confirmed: 11.2.2.2. Supply voltage value calculation (common for all pixels)
...
//Confirmed: 11.2.2.3. Ambient temperature calculation (common for all pixels)
